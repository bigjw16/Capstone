...
8. 알림 시간에 앱이 열리며(기기 정책에 따라 알림 탭 필요 가능) 복약 알람 화면 표시
   - 백그라운드에서는 알림음을 울리지 않고 바로 알람 화면으로 진입
9. 화면에 복약해야 하는 약 이름 표시 + "복약 완료 (알림 종료)" 버튼으로 종료
10. 1분 안에 종료 버튼 미클릭 시 3분 뒤 재알림
11. 복약 완료 시 종료 시간이 복약 기록 화면에 저장
