# Patient Medication Sync System (Web + Flutter + Firebase)
...
> `https.server`가 아니라 `http.server`입니다.
> 아래 오류가 나오면 모듈명을 잘못 입력한 경우입니다.
> `Error while finding module specification for 'https.server'`
