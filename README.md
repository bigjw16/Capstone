...
7. 알림 시간에 앱이 열리며(기기 정책에 따라 알림 탭 필요 가능) 복약 알람 화면 표시
8. 화면에 복약해야 하는 약 이름 표시 + "복약 완료 (알림 종료)" 버튼으로 종료
9. 1분 안에 종료 버튼 미클릭 시 3분 뒤 재알림
10. 복약 완료 시 종료 시간이 복약 기록 화면에 저장
...
4. 앱을 최근 앱에서 닫아도 스케줄은 동작하지만, 일부 제조사(샤오미/화웨이 등)는 자동 시작 허용이 필요
5. 첫 화면 상단 영상 자동재생은 네트워크 연결이 필요
